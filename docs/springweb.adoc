...
----
%dev.quarkus.dev-ui.hosts={{ USER_ID }}-quarkus-workshop-quarkus-webpage.{{ ROUTE_SUBDOMAIN }}
----
...
----
mvn quarkus:dev -Dquarkus.http.host=0.0.0.0 -Dquarkus.analytics.disabled=true -f ${PROJECT_SOURCE}/quarkus-petclinic
----
...
Now, let’s exercise the live reload capabilities of Quarkus. 

Access https://{{ USER_ID }}-quarkus-workshop-quarkus-webpage.{{ ROUTE_SUBDOMAIN }}[Quarkus Petclinic Welcome Page^]. It should now show the following new welcome page that you saw in the Spring Petclinic welcome page:
