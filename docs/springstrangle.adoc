...
Although your Eclipse Che workspace is running on the Kubernetes cluster, it's running with a default restricted _Service Account_ that prevents you from creating most resource types. So we'll log in with your workshop user. Select *Login to OpenShift*:

image::cmd-login.png[login,700]

[NOTE]
====
When you select this, CodeReady Workspaces executes the following command:
