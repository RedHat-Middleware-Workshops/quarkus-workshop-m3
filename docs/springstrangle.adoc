...
image::microservices/vets-service-architecture.png[quarkus, 600]
...
== Deploy Vets Service to OpenShift
...
oc label dc/vets-service app.kubernetes.io/part-of=vets-app --overwrite && \
oc label dc/vets-database app.kubernetes.io/part-of=vets-app --overwrite && \
oc annotate dc/ets-service app.openshift.io/connects-to=vets-database --overwrite
----
