...
----
mvn quarkus:dev -Dquarkus.http.host=0.0.0.0 -Dquarkus.analytics.disabled=true -Ddebug=false -f ${PROJECT_SOURCE}/quarkus-petclinic-vets-service
----
...
Add _quarkus-resteasy-client_ and _quarkus-resteasy-jackson_ extension in VS Code Terminal:

[source,sh,role="copypaste"]
----
mvn quarkus:add-extension -Dextensions="resteasy-client,resteasy-jackson" -f ${PROJECT_SOURCE}/quarkus-petclinic
----
...
[INFO] [SUCCESS] ✅  Extension io.quarkus:quarkus-resteasy-client has been installed
----

* *quarkus-resteasy-client* provides a Quarkus wrapper for the MicroProfile REST Client implementation
...
In the sidebar menu, switch to the Software Catalog using the drop-down under Home.

Select Database in the left-side filter column, then type in `postgres` in the search box, and select the *PostgreSQL (ephemeral)*:
...
* **Version of PostgreSQL Image*: `15-el8`
...
Wait for that command to report `deployment "vets-service" successfully rolled out` before continuing.
...
This will deploy the new vets service to our new project.

Switch to the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-project[Topology View^] using the drop-down under Workloads in the sidebar to view the results:

image::devperspective.png[perspective, 500]

You will see the following resources:
...
Let's do a quick sanity test if new microservices are working well. Select https://petclinic-web-v2-{{ USER_ID }}-project.{{ ROUTE_SUBDOMAIN}}/owners?id=6[Jean Coleman Information^] then you shoud see this:
