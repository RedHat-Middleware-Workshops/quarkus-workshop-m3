...
----
mvn quarkus:dev -Dquarkus.http.host=0.0.0.0 -Ddebug=false -f $CHE_PROJECTS_ROOT/quarkus-workshop-m3-labs/quarkus-petclinic-vets-service
----
...
We will deploy our application to our production environment based on Kubernetes, containers, and [OpenShift](https://openshift.com). This will give us quite a bit of flexibility in how we develop the app going forward.
...
Once logged in, skip the tour by clicking _Skip Tour_, and then click on the name of your project (`{{ USER_ID }}-project`) (it's the only project you have access to):
