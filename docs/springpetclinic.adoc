...
To get started, {{ DS_URL }}[access the CodeReady Workspaces instance^] and log in using the username and password you’ve been assigned (e.g. `{{ USER_ID }}/{{ DS_USER_PASSWORD }}`):
...
Let's import our first project. Click on *Clone Repository* (or type kbd:[F1], enter 'git' and click on the auto-completed _Git Clone.._ )
...
----
cd $CHE_PROJECTS_ROOT/quarkus-workshop-m3-labs && git checkout ocp-4.10
----
