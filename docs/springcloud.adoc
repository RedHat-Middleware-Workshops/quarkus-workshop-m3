...
Click *Create* to establish this ConfigMap resource.
...
Once the rollout is complete, back on the https://petclinic-web-v2-{{ USER_ID }}-project.{{ ROUTE_SUBDOMAIN}}[Welcome Page^] then you shoud see this:
...
[source,sh,role="copypaste"]
----
mvn clean package -f ${PROJECT_SOURCE}/quarkus-petclinic-customers-service && oc rollout status -w deploy/customers-service
----
