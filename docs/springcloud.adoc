...
In this step, we'll use a ConfigMap to handle this capability. Go to the {{ CONSOLE_URL }}/k8s/ns/{{ USER_ID }}-project/core~v1~ConfigMap[Config Maps^] in Developer Console, then select `Create Config Map`:
